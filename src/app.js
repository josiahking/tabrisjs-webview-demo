const {RefreshComposite,WebView, contentView, app, ImageView, Button, TextView, Composite, AlertDialog} = require('tabris');
...
let refreshComp = new RefreshComposite({
	layoutData: 'stretch'
})
.onRefresh(() => {
	refreshComp.refreshIndicator = true;
	if(checkConnection() != Connection.NONE && checkConnection != Connection.UNKNOWN){
		webView.url = webView.url;
		setTimeout(() => {
			refreshComp.refreshIndicator = false;
		},2000);
	}
	refreshComp.refreshIndicator = false;
})
...
if(networkState == Connection.NONE || networkState == Connection.UNKNOWN){
	noNetworkPage.appendTo(refreshComp);
}
else{
	webView.appendTo(refreshComp);
}
...
window.plugins.googleplus.login(
    {
      ...
      'webClientId': '447391654587-q42jr9vsi1po5cinfqp3182nmou91udu.apps.googleusercontent.com', // optional clientId of your Web application from Credentials settings of your project - On Android, this MUST be included to get an idToken. On iOS, it is not required.
